...
<head>
  ...
  <title>Edit Movie - FilmHorizon</title>
  ...
  <div class="container">
    ...
    <div class="movie-form-container">
      <div class="form-header">
        <h2>Edit Movie</h2>
        <p class="text-muted">Update the details of "${movie.title}"</p>
      </div>

      <form action="${pageContext.request.contextPath}/update-movie" method="post" enctype="multipart/form-data">
          ...
          <div class="row">
            <div class="col-md-4 mb-3">
              ...
              <select class="form-select" id="genre" name="genre" required>
                ...
                <option value="Action" ${movie.genre eq 'Action' ? 'selected' : ''}>Action</option>
                <option value="Adventure" ${movie.genre eq 'Adventure' ? 'selected' : ''}>Adventure</option>
                <option value="Animation" ${movie.genre eq 'Animation' ? 'selected' : ''}>Animation</option>
                <option value="Biography" ${movie.genre eq 'Biography' ? 'selected' : ''}>Biography</option>
                <option value="Comedy" ${movie.genre eq 'Comedy' ? 'selected' : ''}>Comedy</option>
                <option value="Crime" ${movie.genre eq 'Crime' ? 'selected' : ''}>Crime</option>
                <option value="Documentary" ${movie.genre eq 'Documentary' ? 'selected' : ''}>Documentary</option>
                <option value="Drama" ${movie.genre eq 'Drama' ? 'selected' : ''}>Drama</option>
                <option value="Family" ${movie.genre eq 'Family' ? 'selected' : ''}>Family</option>
                <option value="Fantasy" ${movie.genre eq 'Fantasy' ? 'selected' : ''}>Fantasy</option>
                <option value="History" ${movie.genre eq 'History' ? 'selected' : ''}>History</option>
                <option value="Horror" ${movie.genre eq 'Horror' ? 'selected' : ''}>Horror</option>
                <option value="Music" ${movie.genre eq 'Music' ? 'selected' : ''}>Music</option>
                <option value="Mystery" ${movie.genre eq 'Mystery' ? 'selected' : ''}>Mystery</option>
                <option value="Romance" ${movie.genre eq 'Romance' ? 'selected' : ''}>Romance</option>
                <option value="Sci-Fi" ${movie.genre eq 'Sci-Fi' ? 'selected' : ''}>Sci-Fi</option>
                <option value="Sport" ${movie.genre eq 'Sport' ? 'selected' : ''}>Sport</option>
                <option value="Thriller" ${movie.genre eq 'Thriller' ? 'selected' : ''}>Thriller</option>
                <option value="War" ${movie.genre eq 'War' ? 'selected' : ''}>War</option>
                <option value="Western" ${movie.genre eq 'Western' ? 'selected' : ''}>Western</option>
              </select>
            </div>
            <div class="col-md-4 mb-3">
              <label for="releaseYear" class="form-label">Release Year</label>
              <input type="number" class="form-control" id="releaseYear" name="releaseYear"
                     min="1900" max="2030" value="${movie.releaseYear}" required>
            </div>
            <div class="col-md-4 mb-3">
              <label for="rating" class="form-label">Rating (1-10)</label>
              <input type="number" class="form-control" id="rating" name="rating"
                     min="1" max="10" step="0.1" value="${movie.rating}" required>
            </div>
          </div>

          <div class="mb-3 checkbox-container">
            <input type="checkbox" id="available" name="available" ${movie.available ? 'checked' : ''}>
            <label for="available">Movie is available for rent</label>
          </div>
        </div>
        ...
        <div class="form-section">
          ...
          <div class="mb-3">
            ...
            <div class="d-flex gap-4">
              <div class="form-check">
                <input class="form-check-input" type="radio" name="movieType" id="regularMovie"
                       value="regular" ${movieType eq 'regular' ? 'checked' : ''}
                       onclick="showMovieTypeSection('regular')">
                <label class="form-check-label" for="regularMovie">
                  Regular Movie
                </label>
              </div>
              <div class="form-check">
                <input class="form-check-input" type="radio" name="movieType" id="newRelease"
                       value="newRelease" ${movieType eq 'newRelease' ? 'checked' : ''}
                       onclick="showMovieTypeSection('newRelease')">
                <label class="form-check-label" for="newRelease">
                  New Release
                </label>
              </div>
              <div class="form-check">
                <input class="form-check-input" type="radio" name="movieType" id="classicMovie"
                       value="classic" ${movieType eq 'classic' ? 'checked' : ''}
                       onclick="showMovieTypeSection('classic')">
          ...
          <div id="newReleaseSection" class="movie-type-section"
               style="${movieType eq 'newRelease' ? 'display: block;' : ''}">
            <div class="mb-3">
              <label for="releaseDate" class="form-label">Release Date</label>
              <c:set var="formattedReleaseDate" value=""/>
              <c:if test="${movieType eq 'newRelease'}">
                <fmt:formatDate value="${movie.releaseDate}" pattern="yyyy-MM-dd" var="formattedReleaseDate"/>
              </c:if>
          ...
          <div id="classicSection" class="movie-type-section"
               style="${movieType eq 'classic' ? 'display: block;' : ''}">
            <div class="mb-3">
              <label for="significance" class="form-label">Historical/Cultural Significance</label>
              <c:set var="significance" value=""/>
              <c:if test="${movieType eq 'classic'}">
                <c:set var="significance" value="${movie.significance}"/>
              </c:if>
              <textarea class="form-control" id="significance" name="significance"
                        rows="3">${significance}</textarea>
            </div>
            <div class="mb-3 checkbox-container">
              <c:set var="hasAwards" value="false"/>
              <c:if test="${movieType eq 'classic'}">
                <c:set var="hasAwards" value="${movie.hasAwards}"/>
              </c:if>
              <input type="checkbox" id="hasAwards" name="hasAwards" ${hasAwards ? 'checked' : ''}>
        ...
        <div class="form-section">
            ...
            <div class="mb-3">
              ...
              <div>
                <img src="${pageContext.request.contextPath}/image-servlet?movieId=${movie.movieId}"
                     class="current-poster" alt="Current Cover Photo">
              </div>
            </div>
          </c:if>
          ...
          <div class="mb-3">
            ...
            <div class="form-text text-muted">
              Leave empty to keep the current cover photo. Recommended size: 500px x 750px. Max file size: 5MB.
            </div>
          </div>
          <div class="preview-container" id="imagePreview" style="display: none;">
            <img id="preview" src="#" alt="New Cover Preview">
          </div>
        </div>

        <div class="d-flex justify-content-between mt-4">
          <div>
            <a href="${pageContext.request.contextPath}/movie-details?id=${movie.movieId}" class="btn btn-outline-light me-3">
          ...
          <a href="${pageContext.request.contextPath}/delete-movie?id=${movie.movieId}" class="btn btn-outline-danger">
