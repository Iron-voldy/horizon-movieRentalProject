...
<head>
  ...
  <title>${selectedGenre} Movies - FilmHorizon</title>
...
<body>
  ...
  <div class="container genre-container">
    ...
    <div class="genre-header position-relative">
      <div class="genre-category">
        <i class="fas fa-film me-2"></i> ${selectedGenre}
      </div>
      <h2>Top ${selectedGenre} Movies</h2>
      <p class="text-muted">Our highest-rated ${selectedGenre} films for the ultimate cinematic experience.</p>

      <a href="${pageContext.request.contextPath}/recommendation-action?action=generate-genre&genre=${selectedGenre}" class="btn btn-sm btn-outline-light refresh-button">
      ...
      <div class="mb-4">
        <div class="d-flex flex-wrap gap-2">
          <c:forEach var="genre" items="${allGenres}">
            <a href="${pageContext.request.contextPath}/genre-recommendations?genre=${genre}"
              class="btn btn-sm genre-btn ${genre eq selectedGenre ? 'active' : ''}">
              ${genre}
            </a>
          </c:forEach>
        </div>
        ...
        <c:when test="${empty recommendations || recommendations.size() == 0}">
          <div class="col-12">
            <div class="empty-recs">
              ...
              <h4>No ${selectedGenre} movies available</h4>
              <p class="text-muted">We don't have any ${selectedGenre} recommendations at the moment.</p>
              <a href="${pageContext.request.contextPath}/recommendation-action?action=generate-genre&genre=${selectedGenre}" class="btn btn-primary mt-3">
                <i class="fas fa-sync-alt me-2"></i> Generate ${selectedGenre} Recommendations
              </a>
            </div>
          </div>
        </c:when>
